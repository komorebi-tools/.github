...
[design-system](https://github.com/komorebi-tools/design-system) で管理しています。
...
**自分用のルールを追加したい場合は、CLAUDE.md や settings.json を自由に編集して OK です。** 組織共通の内容を消さないようにだけ注意してください。
...
色、フォント、コンポーネントなどをビジュアルで確認できるページ → https://komorebi-tools.github.io/design-system/

各ファイルの詳細は [design-system の README](https://github.com/komorebi-tools/design-system) を参照してください。

作業ルールは「はじめかた」のステップ 2 で自動で入ります。

デザインルールはステップ 3 で UI を作るプロジェクトに追加します。
