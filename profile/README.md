...
各メンバーの PC に設定されます。詳細は [Claude Code 組織共通設定](https://www.notion.so/komorebi-inc/Claude-Code-33f13485e9d08044adc8d291c70149dc) を参照してください。**自分用のルールを追加したい場合は、CLAUDE.md や settings.json を自由に編集して OK です。** 組織共通の内容を消さないようにだけ注意してください。
...
各ファイルの詳細は [design-system](https://github.com/komorebi-tools/design-system) の README を参照してください。
...
詳しいルール内容や FAQ は [Claude Code 組織共通設定](https://www.notion.so/komorebi-inc/Claude-Code-33f13485e9d08044adc8d291c70149dc) を参照してください。
...
GitHub での共同開発は「pull (最新を取得) → 変更 → コミット (変更を記録) → push (GitHub に反映)」の繰り返しです。(これらはすべて Claude Code に頼めます)

同じコードを複数人で触るため、手順を決めておかないと誰かの変更を上書きしてしまうことがあります。また、リポジトリは公開設定のため、push したコードはインターネット上の誰でも見られます。API キーなどの機密情報を誤って含めないよう注意が必要です。以下はこれらを防ぐための最低限のルールです。
...
- ローカルの変更を GitHub に反映するときは、Claude Code に「コミットして push して」と伝えてください。他のメンバーも最新の状態を使えるようになります。
