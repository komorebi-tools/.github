...
### 共通ガイドライン (初回セットアップで自動で入る)

各メンバーの PC に設定されます。詳細は [共通ガイドライン](https://www.notion.so/komorebi-inc/Claude-Code-33f13485e9d08044adc8d291c70149dc) を参照してください。**自分用のルールを追加したい場合は、CLAUDE.md や settings.json を自由に編集して OK です。** 共通ガイドラインの内容を消さないようにだけ注意してください。
...
全メンバー共通のセキュリティ&#xB7;作業ルールを適用します。初回のみ実行してください。

Claude Code に以下を伝えてください。

> komorebi-tools/claude-code-org-setup をクローンして、README の手順に従ってセットアップしてください。

詳しいルール内容や FAQ は [共通ガイドライン](https://www.notion.so/komorebi-inc/Claude-Code-33f13485e9d08044adc8d291c70149dc) を参照してください。
...
同じコードを複数人で触るため、手順を決めておかないと誰かの変更を上書きしてしまったり、機密情報を社外に公開してしまうおそれがあります。以下はそれを防ぐための最低限のルールです。
